...
def mnist_dataset_construction():
    ...
    np.save(config_dataset_path + config_data['x_train'], x_train)
    np.save(config_dataset_path + config_data['y_train'], y_train)
    np.save(config_dataset_path + config_data['x_dev'], x_val)
    np.save(config_dataset_path + config_data['y_dev'], y_val)
    np.save(config_dataset_path + config_data['x_test'], x_test)
    np.save(config_dataset_path + config_data['y_test'], y_test)
...
def filter_errors(y_pred_classes, y_pred_probs, y_expected, x_val):
    """
    Function for filtering the erroneous predictions
    ...
    # Filter the errors on predicted probabilities and input values
