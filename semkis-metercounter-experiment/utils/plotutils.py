import csv, itertools, logging, math
...
def plot_confusion_matrix(y_expected, y_predicted, classes, normalize=False, label=""):
    """
    This function prints and plots the confusion matrix.
    Normalization can be applied by setting `normalize=True`.
    """

    cm = ConfusionMatrix(actual_vector=y_expected, predict_vector=y_predicted)

    cm.save_html(configutils.configSectionMap("FOLDERS")['cnn_confusion_path'] + label + "_confusionmatrix_html", color="Crimson")
    cm = confusion_matrix(y_expected, y_predicted)
    plt.imshow(cm, interpolation='none')
    plt.title("Confusion Martix : "+label)
    plt.colorbar()
    tick_marks = np.linspace(0, classes-1, num=classes)
    plt.xticks(tick_marks, rotation=45, fontsize=4)
   ...
   """
    ...
    plt.grid(True)
    path = configutils.configSectionMap("FOLDERS")['cnn_confusion_path'] + label +"_confusionmatrix"
    plt.savefig(path)
    plt.show()

    logging.debug("Successfully Generated Confusion Matrix")
    logging.debug(classification_report(y_expected, y_predicted))
...
def plot_training_statistics(d_loss_logs, g_loss_logs, d_accuracy_logs, g_accuracy_logs):
    ...
    plt.savefig(configutils.configSectionMap("FOLDERS")['cdcgan_training_path'] +
                configutils.configSectionMap("CDCGAN")['cdcgan_performance_loss'])
    plt.show()
    plt.clf()
    plt.cla()
    plt.close()
...
def plot_errors(images, class_predictions, expectations, sorted_predictions, sorted_prediction_indices, filename):
    ...
    for d in single_image_data:
        plot_single_image(d[1], str(d[0]) + "-" + str(d[2]) + "-" + str(d[3]) + "-" + str(d[4]) + ".png",
                          "ID= " + str(d[0]) + "  & PRED= " + str(d[2]) + " & EXP= " + str(d[3]) + " & PROB= " + str(
                              d[4]),
                          filename)
...
def save_and_plot_single_image(image, filename, label, settype):
...
def plot_single_image(image, label, shape):
    plt.title(label)
    plt.imshow(image.reshape(shape), cmap='gray')
    plt.show()


def plot_set_of_images(images, filename, imagenumber_per_row, figsize=(25, 25)):
    print(images.shape)
    number_of_images = len(images) #190
    ncols = imagenumber_per_row #19
    nrows = int(number_of_images/imagenumber_per_row) #10
    ...
    for i in range(nrows):
        for j in range(ncols):
            if counter < number_of_images:
                ax[i, j].imshow((images[counter].reshape(figsize)), cmap='gray')
                ax[i, j].set_xticks([])
                ax[i, j].set_yticks([])
            if counter >= number_of_images:
                fig.delaxes(ax[i, j])
