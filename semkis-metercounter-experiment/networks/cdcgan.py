...
class CDCGAN:
    ...
    def __init__(self):
        ...
        # Create Architecture for GAN components
    ...
    def build_stored_gan(self, trained: bool):
        ...
        """
        # Create Architecture for GAN components
        cdcgan_gen_key = "cdcgan_trained_gen" if trained else "cdcgan_untrained_gen"
        cdcgan_dis_key = "cdcgan_trained_dis" if trained else "cdcgan_untrained_dis"

        self.generator = networkutils.load_model(
            configutils.configSectionMap("FOLDERS")['cdcgan_generator_path'] +
            configutils.configSectionMap("CDCGAN")[cdcgan_gen_key]
        )
        self.discriminator = networkutils.load_model(
            configutils.configSectionMap("FOLDERS")['cdcgan_discriminator_path'] +
            configutils.configSectionMap("CDCGAN")[cdcgan_dis_key]
        )
        ...
        cdcgan = models.Sequential([self.generator, self.discriminator])
        cdcgan.compile(loss='binary_crossentropy', metrics=['accuracy'], optimizer=optimizer)
    ...
    def build_cdcgan(self, generator, discriminator):
        ...
        networkutils.save_model(
            self.generator,
            configutils.configSectionMap("FOLDERS")['cdcgan_generator_path'] +
            configutils.configSectionMap("CDCGAN")['cdcgan_untrained_gen']
        )
        networkutils.save_model(
            self.discriminator,
            configutils.configSectionMap("FOLDERS")['cdcgan_discriminator_path'] +
            configutils.configSectionMap("CDCGAN")['cdcgan_untrained_dis']
        )
        return model
    ...
    def train(self, quantity, labels, min_nb_pdiff, max_nb_pdiff, threshold, min_epoch=0):
        ...
        for epoch in list(map(lambda x: x + 1, range(min_epoch, self.NUM_EPOCH))):
            ...
            print(f"Average Qualities of Generator CSIM : {csim}")
            print(f"Average Qualities of Generator MSE  : {mse}")
            print(f"Average Qualities of Generator SSIM : {ssim}")
            ...
            if epoch == 1 or epoch % 10 == 0:
                ...
                networkutils.save_model(
                    self.generator,
                    configutils.configSectionMap("FOLDERS")['cdcgan_generator_path'] +
                    configutils.configSectionMap("CDCGAN")['cdcgan_trained_gen'] + str(epoch)
                )
                networkutils.save_model(
                    self.discriminator,
                    configutils.configSectionMap("FOLDERS")['cdcgan_discriminator_path'] +
                    configutils.configSectionMap("CDCGAN")['cdcgan_trained_dis'] + str(epoch)
                )
    ...
    def generate_images(self, number_of_images, label):
        ...
        x_synthetic = configutils.configSectionMap("DATA")['x_synthetic']
        y_synthetic = configutils.configSectionMap("DATA")['y_synthetic']

        np.save(dataset_path + x_synthetic + "_" + str(number_of_images), np.array(images))
        np.save(dataset_path + y_synthetic + "_" + str(number_of_images), np.array(label_vectors))
